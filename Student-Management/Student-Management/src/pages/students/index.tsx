...
const StudentPage = () => {
  ...
  const handleModalAction = useCallback(async () => {
    const { ok } = await studentApi.deleteStudent(selectedRecord)
    if (ok) {
      getLecture()
    }
  ...
  const getLecture = async () => {
    const { ok, body } = await studentApi.getStudent()
    if (ok && body) {
      setLectures(body)
    }
  }
  ...
  return (
    <ProtectedRoute roles={[3]}>
      ...
      <SectionMain>
        <SectionTitleLineWithButton title="Student" icon={''}>
          <Button label="Add Student" color="contrast" roundedFull onClick={handleAddClick} small />
                      ...
                      <td className="before:hidden lg:w-1 whitespace-nowrap">
                        <Buttons type="justify-start lg:justify-end" noWrap>
                          <Button
                            color="info"
                            icon={mdiEye}
                            onClick={() => router.push(`/students/${lecture.id}`)}
                            small
                          />
                          <Button
                            color="danger"
